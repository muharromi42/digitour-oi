    ...
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-8">
                ...
                <h1 class="blog-post-title">{{ $penginapan->judul }}</h1>
                ...
                @if ($penginapan->foto)
                    @php
                        ...
                        $fotoData = json_decode($penginapan->foto);
                        ...
                        if (is_array($fotoData)) {
                            ...
                            $fotoPaths[] = $penginapan->foto;
                        }
                    @endphp

                    <!-- Carousel with all images -->
                    <div id="penginapanCarousel" class="carousel slide mb-4" data-bs-ride="carousel">
                        <!-- Indicators/dots -->
                        <div class="carousel-indicators">
                            @foreach ($fotoPaths as $index => $path)
                                <button type="button" data-bs-target="#penginapanCarousel"
                            ...
                            @foreach ($fotoPaths as $index => $path)
                                <div class="carousel-item {{ $index === 0 ? 'active' : '' }}">
                                    <img src="{{ asset('storage/' . $path) }}" class="d-block w-100"
                                        alt="{{ $penginapan->judul }} - Photo {{ $index + 1 }}"
                        ...
                        <button class="carousel-control-prev" type="button" data-bs-target="#penginapanCarousel"
                            data-bs-slide="prev">
                            <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                            <span class="visually-hidden">Previous</span>
                        </button>
                        <button class="carousel-control-next" type="button" data-bs-target="#penginapanCarousel"
                            data-bs-slide="next">
                ...
                <div class="post-meta">
                    <p class="blog-post-meta">
                        ...
                        Published by {{ $penginapan->user->name }}
                    </p>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <dl>
                                ...
                                <dd>{{ $penginapan->waktu_kunjungan }}</dd>
                            </dl>
                        </div>
                        <div class="col-md-6">
                            <dl>
                                <dt>No. telepon</dt>
                                <dd>{{ $penginapan->no_hp }}</dd>
                            </dl>
                        </div>
                    </div>
                </div>
...
if (!str_contains($mapUrl, 'output=embed')) {
    ...
    if (str_contains($mapUrl, 'goo.gl/maps')) {
        // For shortened URLs, we'll need to create a generic embed
                ...
                <div class="mt-4 mb-4">
                    <a href="{{ route('home.penginapan') }}" class="btn btn-secondary">Back to penginapan List</a>
                </div>
            </div>
        </div>
    </div>
